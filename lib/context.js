 ...
 *
 * @param {ContextOptions & ContextDeps} [options]
 * @returns {Context}
 */
  ...
  function createReader(name, state, atr, nativeReader) {
    ...
    const reader = {
      ...
      async connect(shareMode, preferredProtocols) {
        ...
        reader.protocol = nativeReader.protocol;
      },

      async transmit(command, options) {
        if (!nativeReader.connected) throw new Error("Not connected");
        const effectiveAutoGetResponse =
          options?.autoGetResponse ?? (autoGetResponse ? true : undefined);
        if (effectiveAutoGetResponse) {
          return transmitWithAutoResponse(/** @type {any} */ (nativeReader), command, {
            ...options,
      ...
      disconnect(disposition) {
        ...
        reader.protocol = null;
      },

      async reconnect(shareMode, protocol, initialization) {
        if (!nativeReader.connected) throw new Error("Not connected");
        const result = await nativeReader.reconnect(shareMode, protocol, initialization);
        reader.protocol = nativeReader.protocol ?? null;
        return result;
      },
    };
