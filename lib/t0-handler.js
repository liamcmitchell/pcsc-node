 ...
 *
 * @param {number} bytesAvailable Number of bytes to retrieve (Le value)
 * @returns {Buffer} GET RESPONSE command buffer
 */
 ...
 *
 * @param {Buffer} command Original command buffer
 * @param {number} newLe Corrected Le value from SW2
 * @returns {Buffer} New command buffer with corrected Le
 */
 ...
 *
 * @param {{ transmit: (command: Buffer | number[], options?: TransmitOptions) => Promise<Buffer> }} card The card to transmit to
 * @param {Buffer | number[]} command The APDU command
 * @param {TransmitOptions} [options] Transmit options including autoGetResponse
 * @returns {Promise<Buffer>} The response data
 */
...
export { buildGetResponseCommand, correctLeInCommand, transmitWithAutoResponse };
