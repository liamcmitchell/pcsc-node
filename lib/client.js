...
function createClient(options = {}) {
  ...
  async function handleCardInserted(readerName, eventState, atr) {
    ...
    for (let attempt = 0; attempt < maxLookupAttempts; attempt++) {
      ...
      const readerList = context.listReaders();
      reader = readerList.find((r) => r.name === readerName);
      if (reader) break;
    ...
    if (!reader) {
      onError?.(new Error(`Reader not found while handling card insertion: ${readerName}`));
      return;
    }
