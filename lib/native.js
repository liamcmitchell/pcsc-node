 ...
 * @property {(command: Buffer, maxRecvLength?: number) => Promise<Buffer>} transmit
 ...
 * @property {"attached" | "detached" | "changed" | "error" | "ready" | "unready"} type
 * @property {string} name
...
let addon;
try {
  addon = _require("../build/Release/addon.node");
} catch (err) {
  throw new Error(
    "Failed to load native addon. This should have been built by node-gyp on install.",
    { cause: err },
  );
}
